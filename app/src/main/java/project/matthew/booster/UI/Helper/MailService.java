...
public class MailService {
    ...
    public void send() throws AddressException, MessagingException {
        ...
        if (attachments.size() > 0)
            htmlBody = htmlBody.replaceAll("#filename#", attachments.get(0).getFilename());
        if (htmlBody.indexOf("#header#") >= 0)
            htmlBody = htmlBody.replaceAll("#header#", attachments.get(1).getFilename());
        if (htmlBody.indexOf("#footer#") >= 0)
            htmlBody = htmlBody.replaceAll("#footer#", attachments.get(2).getFilename());
